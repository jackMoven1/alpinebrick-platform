...
- **Settlement transaction timeout.** `importSettlementRowsAttempt`'s `prisma.$transaction` (`settlement.ts:456`) uses Prisma's default 5 s interactive timeout, with roughly 4 round trips per row. A large enough report exceeds it and the whole import rolls back. Set `timeout` explicitly or batch.
